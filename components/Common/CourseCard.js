            ...
            return (
                <div className={s.promoCont}>
                    ...
                    <div className={s.desc}>
                        剩<CountDown end={saleEndTime} onEnd={()=>setCountDownFinished(true)}/> &nbsp;恢复原价
                    </div>
                </div>
            )
        }
        ...
        <Link href="/course/detail/[id]" as={`/course/detail/${id}`}>
            <a className={`${s.card} border-b-1px`}>
                ...
                <div className={s.footer}>
                    {/* 教师 */}
                    <div className={s.teacherCont}>{teacherList && teacherList.length > 0
                        ? teacherList.slice(0, 3).map((teacher, idx) => (
                            <div key={`teacher-${idx}-${teacher.name}`} className={s.teacherBox}>
